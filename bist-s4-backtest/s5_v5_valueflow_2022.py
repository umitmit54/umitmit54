...
def parse_money_tl(t):
    # Conservative monetary parser: only values explicitly followed/preceded by TL/TRY/USD/EUR.
    vals=[]
    pats=[
      r'(?P<n>\d{1,3}(?:[\.\s]\d{3})+(?:,\d+)?)\s*(?P<c>tl|try|usd|eur|dolar|euro|avro)',
      r'(?P<c>tl|try|usd|eur)\s*(?P<n>\d{1,3}(?:[\.\s]\d{3})+(?:,\d+)?)'
    ]
    fx={'tl':1.0,'try':1.0,'usd':18.0,'dolar':18.0,'eur':19.0,'euro':19.0,'avro':19.0} # fixed coarse proxy; only used for ranking magnitude
    for p in pats:
        for m in re.finditer(p,t,re.I):
            raw=m.group('n').replace(' ','').replace('.','').replace(',','.')
            try: vals.append(float(raw)*fx[m.group('c').lower()])
            except: pass
    return max(vals) if vals else 0.0

def event_kind_and_direction(d,age):
    t=txt(d); ev=0.0; flowdir=0.0
    if any(x in t for x in ['sorumluluk beyanı','faaliyet raporu','finansal rapor','kurumsal yönetim','sürdürülebilirlik raporu']):return 0.0,0.0,0.0
    ...
    if buy and not sell:flowdir=1.0
    elif sell and not buy:flowdir=-1.0
    elif buy and sell:flowdir=0.15
    if re.search(r'yeni iş|iş ilişkisi|sözleşme|sipariş|ihale|proje',t):ev+=4
    if re.search(r'yatırım|kapasite art|tesis|üretim|ruhsat|lisans',t):ev+=3
    if re.search(r'ortaklık|iş birliği|işbirliği|stratejik',t):ev+=2.5
    if re.search(r'endeks|msci|ftse',t):ev+=3.5
    if re.search(r'bedelsiz|sermaye artırımı',t):ev+=1.0
    if re.search(r'finansal duran varlık edinimi|devral|satın alma',t):ev+=2
    if re.search(r'iflas|konkordato|tasfiye',t):ev-=6
    if re.search(r'ceza|yaptırım|faaliyet.*durdur',t):ev-=3
    decay=max(0.25,1.0-0.10*max(age,0))
    return ev*decay,flowdir*decay,parse_money_tl(t)*decay
...
def features(h):
    ...
    value=(q.close.astype(float)*q.volume.astype(float)); adv20=float(value.iloc[:-1].mean()) if len(value)>1 else np.nan
    value1=float(value.iloc[-1]/adv20) if np.isfinite(adv20) and adv20>0 else np.nan
    value5=float(value.iloc[-5:].mean()/adv20) if np.isfinite(adv20) and adv20>0 else np.nan
    # signed accumulation proxy: positive-return days weighted by traded value minus negative-return days
    rets=c.pct_change(); signed=np.sign(rets.iloc[-10:]).fillna(0)*value.iloc[-10:]
    flow10=float(signed.sum()/(value.iloc[-10:].sum()+1e-9))
    tech=0
    ...
    return dict(ret3=r3,ret5=r5,ret20=r20,vol1=vol1,dist_high=dh,adv20=adv20,value1=value1,value5=value5,flow10=flow10,tech=tech,reversal=rev)
def wret(d):
...
def main():
    weeks=pd.DataFrame({'week_start':pd.date_range('2022-01-03','2022-12-26',freq='W-MON')})
    weeks['week_end']=weeks.week_start+pd.Timedelta(days=4); weeks=weeks[weeks.week_end<=pd.Timestamp('2022-12-31')].copy(); weeks['test_id']=['H22-%02d'%(i+1) for i in range(len(weeks))]
    u=universe(); print('Universe',len(u)); cache={}
    for i,s in enumerate(u,1):
        cache[s]=fetch_stock(s,'2021-11-01','2023-01-06')
    ...
    for _,w in weeks.iterrows():
        ...
        evscore={s:0.0 for s in u}; flowdir={s:0.0 for s in u}; money={s:0.0 for s in u}; evcount={s:0 for s in u}
        disclosures=kap_window(ks,ws-pd.Timedelta(days=7),ws)
        for d in disclosures:
            try:dt=pd.to_datetime(d.get('publishDate'),format='%d.%m.%Y %H:%M:%S')
            except:continue
            if dt>=cutoff:continue
            es,fd,amt=event_kind_and_direction(d,(cutoff-dt).total_seconds()/86400)
            for s in [x for x in tickers(d) if x in evscore]:
                evscore[s]+=es; flowdir[s]+=fd; money[s]=max(money[s],amt); evcount[s]+=1
        rows=[]
        for s in u:
            f=features(sl(cache[s],ws-pd.Timedelta(days=50),ws-pd.Timedelta(days=1)))
            if f is None:continue
            amount_adv=(money[s]/f['adv20']) if np.isfinite(f['adv20']) and f['adv20']>0 else 0.0
            amount_adv=np.clip(amount_adv,0,25)
            gate=(f['tech']>=4) or (f['reversal']==1) or (evscore[s]>=2) or (flowdir[s]>0.5) or (f['value1']>=1.8) or (f['value5']>=1.4)
            rows.append({'symbol':s,**f,'event':evscore[s],'flowdir':flowdir[s],'money_tl_proxy':money[s],'amount_adv':amount_adv,'event_count':evcount[s],'gate':gate,'week_return':wret(sl(cache[s],ws,we))})
        r=pd.DataFrame(rows)
        if r.empty:continue
        cand=r[r.gate].copy()
        if len(cand)<20:cand=r.nlargest(30,'tech').copy()
        # V5 frozen first-principles value/flow ranker. All variables available before week start.
        for col in ['ret20','ret3','flowdir','tech','event','amount_adv','value1','value5','flow10']:
            cand[col+'_p']=pr(cand[col].replace([np.inf,-np.inf],np.nan).fillna(cand[col].median() if cand[col].notna().any() else 0))
        cand['count_p']=pr(cand.event_count.fillna(0))
        cand['v5_score']=(0.20*cand.ret20_p+0.08*cand.ret3_p+0.13*cand.flowdir_p+0.08*cand.tech_p+
                          0.10*cand.event_p+0.16*cand.amount_adv_p+0.10*cand.value1_p+0.06*cand.value5_p+
                          0.12*cand.flow10_p-0.03*cand.count_p)
        cand=cand.sort_values(['v5_score','amount_adv','flow10','ret20'],ascending=False).reset_index(drop=True); cand['rank']=np.arange(1,len(cand)+1); ranks.append(cand.assign(test_id=w.test_id))
    ...
    pd.DataFrame([{'weeks':len(sm),'pool_recall':sm.leader_in_pool.mean(),'top5':sm.leader_top5.mean(),'top10':sm.leader_top10.mean(),'top20':sm.leader_top20.mean(),'median_rank':sm.leader_rank.dropna().median(),'avg_portfolio_return':sm.portfolio_return.mean(),'note':'2022 untouched holdout. V5 uses pre-week OHLCV traded-value flow proxies and KAP explicit money normalized by trailing ADV. CURRENT_XU100_PROXY. FX magnitude conversion is coarse ranking proxy only.'}]).to_csv(OUT/'stats.csv',index=False)

...
OUT=ROOT/'output_s5v7_2020'; OUT.mkdir(exist_ok=True)
...
def bench_regime(bench,ws):
    ...
    b5=pct(c.iloc[-6],c.iloc[-1]); b20=pct(c.iloc[-21],c.iloc[-1])
    weak=(np.isfinite(b20) and b20<0) or (np.isfinite(b5) and b5<-0.01)
    return ('weak' if weak else 'strong'),b5,b20

def main():
    weeks=pd.DataFrame({'week_start':pd.date_range('2020-01-06','2020-12-28',freq='W-MON')})
    weeks['week_end']=weeks.week_start+pd.Timedelta(days=4); weeks=weeks[weeks.week_end<=pd.Timestamp('2020-12-31')].copy(); weeks['test_id']=['H20-%02d'%(i+1) for i in range(len(weeks))]
    u=universe(); print('Universe',len(u)); cache={}
    for i,s in enumerate(u,1):
        cache[s]=fetch_stock(s,'2019-11-01','2021-01-08')
        if i%10==0:print('prices',i,'/',len(u))
    bench=fetch_index('2019-11-01','2021-01-08'); ks=session(); summaries=[]; ranks=[]
    ...
    row['note']='2020 untouched holdout. V7 regime rule and weights frozen from 2021 diagnostics; all regime inputs are pre-week XU100 data. CURRENT_XU100_PROXY.'

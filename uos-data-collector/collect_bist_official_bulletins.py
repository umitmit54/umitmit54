...
KEEP_META=['TARIH','ISLEM  KODU','BULTEN ADI','PAZAR','YAPISAL BAZDA PIYASA ALT BOLUMU','ENSTRUMAN GRUBU','ENSTRUMAN TIPI','ENSTRUMAN SINIFI','ISLEM YONTEMI','BIST 100 ENDEKS','BIST 30 ENDEKS','BRUT TAKAS','OZSERMAYE HALI','GECICI DURDURMA','ACIGA SATIS']
...
def main():
 ...
 for i,d in enumerate(daterange(a.year),1):
  ...
  try:
   ...
   # First data row is bilingual English header.
   ...
   n=pd.DataFrame({'date':pd.to_datetime(f['TARIH'],errors='coerce').dt.strftime('%Y-%m-%d'),'instrument_id':f['instrument_id']})
   for src,dst in NUMERIC_MAP.items(): n[dst]=pd.to_numeric(f[src],errors='coerce') if src in f.columns else pd.NA
   for src in ['BIST 100 ENDEKS','BIST 30 ENDEKS','BRUT TAKAS','OZSERMAYE HALI','GECICI DURDURMA','ACIGA SATIS','PAZAR','YAPISAL BAZDA PIYASA ALT BOLUMU','ISLEM YONTEMI']:
    if src in f.columns: n[src.lower().replace(' ','_').replace('ı','i')]=f[src].values
